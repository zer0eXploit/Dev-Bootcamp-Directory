...
describe('Individual Bootcamp Route Tests. [PUT]', function () {
  ...
  it('Respond with 401 if Authorization header is not present.', async function () {
    const statusCode = await (await request.put(`${endPoint}`)).status;
  ...
  it('A user shall not update a course.', async function () {
    await request
      .put(`${endPoint}`)
  ...
  it('Respond with a 404 if invalid course ID is supplied.', async function () {
    const invalid = '8d713995b7wef21c3bb38sfc1f5d0';
    const endPoint = `/api/v1/courses/${invalid}`;
    await request
      .put(`${endPoint}`)
      .set('Authorization', `Bearer ${process.env.PUBLISHER_TOKEN}`)
      .expect(404);
  ...
  it('Publisher shall be able to update their course.', async function () {
    await request
      .put(`${endPoint}`)
      .set('Authorization', `Bearer ${process.env.PUBLISHER_TOKEN}`)
      .expect(200);
  ...
  it('Publisher can only update their own course.', async function () {
  ...
  it('Admin can update any course.', async function () {
    const alreadyCreated = '5d725a4a7b292f5f8ceff788';
    const endPoint = '/api/v1/courses';
    await request
      .put(`${endPoint}/${alreadyCreated}`)
